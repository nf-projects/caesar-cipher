...
export function caesarEncrypt(input: string, key: number): string {
  ...
  for (let i = 0; i < inputArray.length; i++) {
    const charCode: number = getCharNum(inputArray[i]);
    
    const encryptedCharCode = getIncrementedCharNum(charCode, key);

    const encryptedChar = getCharFromNum(encryptedCharCode);
